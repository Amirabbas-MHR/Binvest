...
class App:
    ...
    ############################################################################################################################
    def main(self):
        st.set_page_config(layout="wide", page_title="Binvest", page_icon="💵")
        #st.sidebar.title(""" Choose a service:""")
        tab = st.sidebar.radio("""- Choose a tab:""", list(self.tabs.keys())) #tabs.keys() is a list of tabs
        self.tabs[tab]() #running the related function
